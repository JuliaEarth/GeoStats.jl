@testset "Kriging estimators" begin
  ...
  γ_f = GaussianVariogram{Float32,Float32,EuclideanDistance}(sill=1., range=1., nugget=0.)
  X_d  = Float64.(X_f)
  z_d  = Float64.(z_f)
  xₒ_d = Float64.(xₒ_f)
  γ_d = GaussianVariogram{Float64,Float64,EuclideanDistance}(sill=1., range=1., nugget=0.)
  ...
  driftkrig_f = ExternalDriftKriging(X_f, z_f, γ_f, [x->Float32(1)])
  ...
  driftkrig_d = ExternalDriftKriging(X_d, z_d, γ_d, [x->Float64(1)])
